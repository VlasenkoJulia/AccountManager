...
public class PasswordControllerTest {
    ...
    private UserEntity userEntityWithoutToken = new UserEntity("username", "password", "email@demo.com", null, "password");
    ...
    private UserEntity userEntityWithToken = new UserEntity("name", "password", "email@demo.com", "token", "password");
    ...
    public void sendResetMessage_ValidEmail_ShouldReturnSuccessMessage() throws Exception {
        when(userService.getByEmail("email@demo.com")).thenReturn(userEntityWithoutToken);
        MvcResult result = mockMvc.perform(post("/forgotPassword?email=email@demo.com"))
                .andExpect(status().isOk())
                .andReturn();
        String resetToken = userEntityWithoutToken.getResetToken();
    ...
    public void resetPasswordForm_ShouldRenderResetPasswordForm() throws Exception {
        when(userService.getByResetToken("token")).thenReturn(userEntityWithToken);
        mockMvc.perform(get("/reset?token=token"))
                .andExpect(status().isOk())
                ...
                .andExpect(model().attribute("userName", userEntityWithToken.getUserName()))
                .andExpect(model().attribute("email", userEntityWithToken.getEmail()))
                .andExpect(model().attribute("token", userEntityWithToken.getResetToken()));
    }

    @Test
    public void resetPassword_InvalidUser_ShouldReturnErrorDto() throws Exception {
        when(userService.resetPassword(userEntityWithoutToken)).thenThrow(new InputParameterValidationException(EXCEPTION_MESSAGE));
    ...
    public void resetPassword_ValidUser_ShouldReturnSuccessMessage() throws Exception {
        when(userService.resetPassword(userEntityWithToken)).thenReturn("Password changed successfully");
